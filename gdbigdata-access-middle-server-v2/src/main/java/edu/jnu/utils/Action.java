...
import edu.jnu.config.OSU.PrivateKey;
...
//	}
	 ...
	 * @param pk 公钥
	 * @param s 层数
	 * @param randomSequenceSize 向量大小
	 * @param i 1元素的下标
	 * @return 加密后的tao向量
	 */
	public static CipherText[] createTaoVector(final PublicKey pk, final int s, int randomSequenceSize, int i) {
		// 创建一个randomSequenceSize大小的CipherText数组
		CipherText[] taos = new CipherText[randomSequenceSize];
		taos[i] = Action.Enc(pk, BigInteger.ONE, s+2, s+2);
		// 初始化密文CipherText数组的密文全为0，除第i个为1
		// 加密数组中所有密文
		taos = Arrays.stream(taos).parallel().map(tao -> {
			if(tao == null) {
				return Action.Enc(pk, BigInteger.ZERO, s + 2, s + 2);
			} else {
			    return tao;
			}
